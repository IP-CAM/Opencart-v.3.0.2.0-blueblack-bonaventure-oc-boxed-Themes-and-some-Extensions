	...
	public function addOrderStatus($data) {
		foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET order_status_id = '" . (int)$order_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', jclr_bg = '" . $this->db->escape($data['j_backgrond_color']) . "', jclr_text = '" . $this->db->escape($data['j_text_color']) . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', jclr_bg = '" . $this->db->escape($data['j_backgrond_color']) . "', jclr_text = '" . $this->db->escape($data['j_text_color']) . "'");
	...
	public function getOrderStatuses($data = array()) {
		...
		if ($data) {
			...
			if (!$order_status_data) {
				$query = $this->db->query("SELECT order_status_id, name, jclr_bg, jclr_text FROM " . DB_PREFIX . "order_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
